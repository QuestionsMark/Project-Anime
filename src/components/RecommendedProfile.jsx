...
const RecommendedProfile = ({username, avatar, likes, background, link}) => {
    return ( 
        <Link to={`/profile/${link}`} className="RP__profile" style={{backgroundImage: `url('http://localhost:9000/images/${background}')`, backgroundPosition: 'center',
        backgroundSize: 'cover'}}>
            <div className="RP__curtain"></div>
            <div className="RP__imgWrapper">
                <img src={`http://localhost:9000/images/${avatar}`} alt="avatar" className="img"/>
            </div>
